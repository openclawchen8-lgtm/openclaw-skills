...
用法：
  python3 -m ideas2tasks.task_completion_hook <task_file>
  python3 -m ideas2tasks.task_completion_hook <task_file> --status in-progress
  python3 -m ideas2tasks.task_completion_hook --scan-done
"""
...
def scan_and_sync_done_tasks():
    ...
    for project_dir in tasks_dir.iterdir():
        ...
        projects_scanned += 1
        tasks_subdir = project_dir / "tasks"
        
        if not tasks_subdir.exists():
            continue
        
        # 找出所有 done 的 tasks
        for task_file in tasks_subdir.glob("T*.md"):
