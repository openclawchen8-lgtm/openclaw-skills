...
用法：
  python3 -m ideas2tasks.sync_status                    # 同步所有專案
  python3 -m ideas2tasks.sync_status --project xxx      # 同步特定專案
  python3 -m ideas2tasks.sync_status --dry-run          # 預覽模式
  python3 -m ideas2tasks.sync_status --fix-history      # 修復歷史不一致
"""
...
def fix_history():
    ...
    # 寫入修復記錄
    sync_log = Path(__file__).parent.parent / "scripts" / "sync_history.json"
